    ...
    if [ ${group} == "train" ]; then
        ...
        do
            ...
            do
                ...
                ./pipline_poly --edge_no="$num_side" --radius="${radius}" --hight="${height}" --freq_max="${freq_max}"\
                    --output_file_path="${base}/${group}/mesh/poly_${curr_idx}.obj"
            done
        done
    fi
...
do
    ...
    if [ ${group} == "validation" ]; then
        #echo "generating mesh for val"
        numEachType=numPolyEachType_val
        for (( num_side=3; num_side<=10; num_side++ ))
        do
            for (( idx=0; idx<numEachType; idx++ ))
            do
                #echo "idx: ${idx}"
